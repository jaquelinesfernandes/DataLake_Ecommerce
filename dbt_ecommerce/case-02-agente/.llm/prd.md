...
No Telegram, abrir `@Data_Ecommerce_bot` e enviar `/start`. O `CHAT_ID` é salvo automaticamente.
...
# Diário às 8h:
0 8 * * * cd Z:/ProjetosML/DataLake_Ecommerce/dbt_ecommerce/case-02-agente && python agente.py >> /tmp/agente.log 2>&1
```
